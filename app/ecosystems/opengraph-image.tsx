...
export default async function EcosystemsOgImage() {
  let ecosystems: Awaited<ReturnType<typeof prisma.ecosystem.findMany>> = [];
  
  try {
    ecosystems = await prisma.ecosystem.findMany({
      orderBy: [
        { featured: 'desc' },
        { displayOrder: 'asc' }
      ],
    ...
    console.error('[ecosystems/opengraph-image] Failed to fetch ecosystems:', error);
    ...
    (
      ...
      >
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 24 }}>
          {ecosystems.length > 0 ? ecosystems.map((ecosystem) => (
            ...
            <div style={{ background: '#111C2E', borderRadius: 24, padding: 24, flex: '1 1 100%' }}>
              <div style={{ fontSize: 28, fontWeight: 600 }}>Dewrk Ecosystems</div>
              <div style={{ marginTop: 12, fontSize: 20, opacity: 0.72 }}>Explore modular & L2 ecosystems</div>
            </div>
          )}
        </div>
